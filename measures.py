...
def sample_fld(x: torch.Tensor, estimator: str = 'var'):
    '''
    x: torch.Tensor, shape (n, d, s)
    OPTIONAL:
    ...
    if estimator == 'cov':
        ...
        fld, s_b, s_w = var_criterion(within, between)

    return  (fld.item(), s_b.item(), s_w.item())
...
def class_fld(x: torch.Tensor, y: torch.Tensor, estimator: str = 'var'):
    ...
    crit, s_b, s_w = sample_fld(x, estimator=estimator)

    return crit, s_b, s_w

def sample_mse(x: torch.Tensor):
    '''
    x: torch.Tensor, shape (n, d, s)
    '''
    N, D, S = x.shape
