...
parser.add_argument('--window_size', type=int, default=3, metavar='N',
                     help='window size for FLD evaluation (default: 3)')
...
rel_fld_sample_dt = np.zeros(T)
rel_fld_class_dt = np.zeros(T) 
abs_fld_sample_dt = np.zeros(T)
abs_fld_class_dt = np.zeros(T) 
...
for i, t_value in enumerate(ts):
    ...
    rel_fld_sample[i], _, _ = measures.sample_fld(rel_latent_samples)
    rel_fld_class[i], _, _ = measures.class_fld(rel_latent_samples, targets)

    abs_fld_sample[i], _, _ = measures.sample_fld(abs_latent_samples)
    abs_fld_class[i], _, _ = measures.class_fld(abs_latent_samples, targets)
...
rel_latent_samples = torch.cat((rel_latent_samples[:,:, :args.window_size//2], rel_latent_samples, rel_latent_samples[:,:, -args.window_size//2:]), dim=2)
abs_latent_samples = torch.cat((abs_latent_samples[:,:, :args.window_size//2], abs_latent_samples, abs_latent_samples[:,:, -args.window_size//2:]), dim=2)

print('\n Windowed evaluation \n')
columns = ['i', 't', 'rel fld sample dt', 'rel fld class dt', 'abs fld sample dt', 'abs fld class dt']

for i, t_value in enumerate(ts):
    rel_fld_sample_dt[i], _, _ = measures.sample_fld(rel_latent_samples[:,:, i:i+args.window_size])
    rel_fld_class_dt[i], _, _ = measures.class_fld(rel_latent_samples[:,:, i:i+args.window_size], targets)

    abs_fld_sample_dt[i], _, _ = measures.sample_fld(abs_latent_samples[:,:, i:i+args.window_size])
    abs_fld_class_dt[i], _, _ = measures.class_fld(abs_latent_samples[:,:, i:i+args.window_size], targets)

    values = [i, t_value, rel_fld_sample_dt[i], rel_fld_class_dt[i], abs_fld_sample_dt[i], abs_fld_class_dt[i]]
    table = tabulate.tabulate([values], columns, tablefmt='simple', floatfmt='10.4f')
...
np.savez(
    ...
    ts=ts,
    cumulative_relative_class_fld=rel_fld_class,
    cumulative_relative_sample_fld=rel_fld_sample,
    cumulative_absolute_class_fld=abs_fld_class,
    cumulative_absolute_sample_fld=abs_fld_sample,
    test_accuracy=test_acc,
    ensemble_accuracy=ens_acc,
    dt_relative_sample_fld=rel_fld_sample_dt,
    dt_relative_class_fld=rel_fld_class_dt,
    dt_absolute_sample_fld=abs_fld_sample_dt,
    dt_absolute_class_fld=abs_fld_class_dt,
)
